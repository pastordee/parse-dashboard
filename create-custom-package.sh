...
cp .gitignore "$CUSTOM_DIR/" 2>/dev/null || echo "No .gitignore found, skipping..."
cp babel.config.js "$CUSTOM_DIR/"
cp eslint.config.js "$CUSTOM_DIR/"
cp jsconfig.json "$CUSTOM_DIR/"

# Copy any other config files
cp Dockerfile* "$CUSTOM_DIR/" 2>/dev/null || echo "No Dockerfile found, skipping..."

cd "$CUSTOM_DIR"

echo "🔧 Updating package.json..."

# Create a new package.json with updated information
cat > package.json << EOF
{
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/$AUTHOR_NAME/parse-dashboard-analytics.git"
  },
  "homepage": "https://github.com/$AUTHOR_NAME/parse-dashboard-analytics",
  "bugs": {
    "url": "https://github.com/$AUTHOR_NAME/parse-dashboard-analytics/issues"
  },
...
$(cat ../package.json | jq '.dependencies' | sed 's/^/  "dependencies": /')
,
$(cat ../package.json | jq '.devDependencies' | sed 's/^/  "devDependencies": /')
,
$(cat ../package.json | jq '.scripts' | sed 's/^/  "scripts": /')
,
$(cat ../package.json | jq '.jest' | sed 's/^/  "jest": /')
,
$(cat ../package.json | jq '.husky' | sed 's/^/  "husky": /')
,
$(cat ../package.json | jq '."lint-staged"' | sed 's/^/  "lint-staged": /')
}
EOF
